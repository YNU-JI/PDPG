...
def add_noise(img_shape):
    # 生成随机噪声的强度
    # strength = np.random.uniform(0, 0.3)
    # img_shape.append(1) # 加上通道数
    height, width = img_shape[:2]
    # print(img_shape)
    # # 生成随机噪声
    # mask = np.random.normal(0, strength, img_shape)
    # mask = torch.from_numpy(noise)
    eps = random.uniform(8/255, 0.1)  # 随机从8/255和0.1中生成概率

    # 生成随机矩阵，以prob_1的概率生成1
    mask1 = np.random.choice([0, 1], size=(height, width, 1), p=[1, 0]).astype('uint8')
    mask2 = np.random.choice([0, 1], size=(height, width, 1), p=[0, 1]).astype('uint8')
    # mask.dtype = 'uint8'
    return mask1, mask2,eps
...
#
#     image_size = [32, 32]
#     print("1",image_size[:2])
#     # h, w = image_size
#     # image_size.append(1)
#     mask1 = get_irregular_mask(image_size, )
#     # mask2 = bbox2mask(image_size,  (h//4, w//4, h//2, w//2))
#     mask1, mask2 = add_noise(image_size)
#     print(mask2.sum())
#     # mask.reshape(image_size)
#     # mask.dtype = 'uint8'
#     # mask = torch.from_numpy(mask).permute(2, 0, 1)
#     print(mask.shape)
#     # print(mask[0][1][0], mask[0][1][1], mask[0][1][2])
#     # print(mask.sum()/(h*w))
#     # print(mask.dtype,mask1.dtype,mask2.dtype)
#     # prob_1 = random.uniform(0.1, 0.3)

    # 生成随机矩阵
    # random_matrix = np.random.choice([0, 1], size=[32, 32], p=[1 - prob_1, prob_1])

    # print(prob_1,random_matrix.sum()/(32 * 32))
