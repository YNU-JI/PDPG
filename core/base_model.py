...
class BaseModel():
            ...
            if self.epoch % self.opt['train']['val_epoch'] == 0:
                ...
                if self.val_loader is None:
                    ...
                    for key, value in val_log.items():
                        self.logger.info('{:5s}: {}\t'.format(str(key), value))
                self.logger.info("\n------------------------------Validation End------------------------------\n\n")
            self.logger.info('best_acc:{}, cur_acc:{}'.format(self.best_acc, self.correct))
            if self.epoch % self.opt['train']['save_checkpoint_epoch'] == 0 or self.correct > self.best_acc:
                self.logger.info('Saving the self at the end of epoch {:.0f}'.format(self.epoch))
                self.save_everything()
                if self.correct > self.best_acc:
                    self.best_acc = self.correct
            torch.cuda.empty_cache()
            torch.cuda.synchronize()  # 等待所有 GPU 操作完成
            torch.cuda.empty_cache()  # 再次清理缓存
    ...
    def save_network(self, network, network_label):
        ...
        self.logger.info('best_acc:{}, cur_acc:{}'.format(self.best_acc, self.correct))
        # save_filename = 'best_{}.pth'.format(network_label)
        # save_path = os.path.join(self.opt['path']['checkpoint'], save_filename)
        if self.opt['global_rank'] != 0:
            return
        if self.correct > self.best_acc:
            self.logger.info('best_acc:{}, Saving network'.format(self.best_acc))
            save_filename = 'best_{}.pth'.format(network_label)
            save_path = os.path.join(self.opt['path']['checkpoint'], save_filename)
            if isinstance(network, nn.DataParallel) or isinstance(network, nn.parallel.DistributedDataParallel):
                network = network.module
            ...
            save_filename = 'last_{}.pth'.format(network_label)
            save_path = os.path.join(self.opt['path']['checkpoint'], save_filename)
