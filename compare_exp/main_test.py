...
def main_worker(gpu, ngpus_per_node):
    ...
    train_x_dataset = NpyDataset(train_x_data_root, 'imagenet50')
    train_adv_dataset = NpyDataset(train_adv_data_root, 'imagenet50')
    ...
    denoiser.denoiser.load_state_dict(torch.load(f"{method}_{DATA_TYPE}.pth"))
    ...
    #     y = x_y.to(device)
        ...
        # labels = x.to(device)
        save_tensor_as_image(x, DATA_TYPE, f'/home/special/user/jijun/Adv_img/{DATA_TYPE}/test/resnet50/aaave_image/{method}_{count}')
        # print(labels.shape)
